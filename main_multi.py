"""
Gate.io Futures Trading Bot - Multi-Symbol Trading
Grid Trading Strategy for BTC and ETH
...
class MultiSymbolTradingBot:
    def __init__(self):
        """Initialize multi-symbol trading bot"""
        self.exchange = GateioFutures(testnet=config.TESTNET)

        # Symbols to trade
        self.symbols = ['BTC_USDT', 'ETH_USDT']

        # Strategy for each symbol
        self.strategies = {}
        for symbol in self.symbols:
            self.strategies[symbol] = GridTradingStrategy(
                num_grids=30,
                range_pct=10.0,
                profit_per_grid=0.3,
                max_positions=10,
    ...
    def calculate_order_size(self, symbol, capital_pct):
        ...
        try:
            ...
            # Gate.io: BTC and ETH both use 0.0001 contract size
    ...
    def execute_trade(self, symbol, signal, capital_pct):
        ...
        if isinstance(signal, dict):
            ...
            print(f"\n[{symbol}] Signal: {side.upper()} | TP: {tp}% | SL: {sl}%")
        else:
            side = signal
            tp = 3.0
            sl = 2.0
